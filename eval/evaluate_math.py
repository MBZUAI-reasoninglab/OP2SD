    ...
    if lora_adapter_path is not None:
        ...
        if adapter_path.exists():
            ...
            raise FileNotFoundError(
                f"No LoRA weights found in requested checkpoint: {lora_adapter_path}"
            )
...
def evaluate_math500(
    ...
    Args:
        ...
        max_model_len: Maximum model context length used by vLLM
        tensor_parallel_size: Number of GPUs used for tensor parallel inference
        ...
        dataset_path: Optional local JSON/JSONL file with problem/answer rows
        base_model_name: Base model name for logging
        enable_thinking: Whether to use thinking mode
        seed: Optional vLLM sampling seed for reproducible decoding
        checkpoint_dir: Optional adapter/checkpoint identity saved with the result
    """
    if checkpoint_dir is not None and lora_request is None:
        raise ValueError(
            "checkpoint_dir was requested but no LoRA request was provided; refusing to evaluate the base model under a checkpoint label"
        )
    ...
    print(f"Sampling Seed: {seed if seed is not None else 'vLLM default'}")
    print(f"{'='*70}\n")
    ...
    if dataset_path:
        ...
        if not resolved_dataset_path.is_file():
            raise FileNotFoundError(
                f"Custom evaluation dataset does not exist: {resolved_dataset_path}"
            )
        dataset_path = str(resolved_dataset_path)
        dataset_sha256 = hashlib.sha256(
            resolved_dataset_path.read_bytes()
        ).hexdigest()
        dataset = load_dataset(
            "json",
            data_files={"test": str(resolved_dataset_path)},
            split="test",
        )
        missing_columns = {
            column
            for column in ("problem", "answer")
            if column not in dataset.column_names
        }
        if missing_columns:
            raise ValueError(
                "Custom evaluation dataset is missing required columns: "
                + ", ".join(sorted(missing_columns))
            )
        print(
            f"Loaded custom dataset {resolved_dataset_path} with "
    ...
    for example in dataset:
        ...
        if dataset_path:
            ...
            question_id = example.get(
                "id",
                example.get("record_id", example.get("idx", None)),
            )
        elif dataset_name.lower() == "amo-bench":
    ...
    if output_file:
        ...
        summary = {
            ...
            "checkpoint_loaded": lora_request is not None,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="math500",
        help="Dataset label or built-in dataset alias (default: math500)",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default=None,
        help="Optional local JSON/JSONL evaluation file with problem and answer columns.",
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="vLLM sampling seed. Omit to use the vLLM default.",
    )
    ...
    if args.dataset_path is not None:
        resolved_dataset_path = Path(args.dataset_path).expanduser().resolve()
        if not resolved_dataset_path.is_file():
            raise FileNotFoundError(
                f"Custom evaluation dataset does not exist: {resolved_dataset_path}"
            )
        args.dataset_path = str(resolved_dataset_path)
        dataset_file_sha256 = hashlib.sha256(
            resolved_dataset_path.read_bytes()
        ).hexdigest()
        dataset_preflight = load_dataset(
            "json",
        ...
        missing_columns = {
            column
            for column in ("problem", "answer")
            if column not in dataset_preflight.column_names
        }
        if missing_columns:
            raise ValueError(
                "Custom evaluation dataset is missing required columns: "
        ...
        if len(dataset_preflight) == 0:
            raise ValueError("Custom evaluation dataset is empty.")

    # Validate and prepare the requested LoRA checkpoint before allocating GPU memory.
    lora_request = None
    if args.checkpoint_dir is not None:
        ...
        if not checkpoint_path.is_dir():
            ...
            print("This directory does not exist or is not a directory.")
            print(f"{'='*70}\n")
            raise SystemExit(1)

        adapter_safetensors = checkpoint_path / "adapter_model.safetensors"
        adapter_bin = checkpoint_path / "adapter_model.bin"
        if not (adapter_safetensors.is_file() or adapter_bin.is_file()):
            print(f"\n{'='*70}")
            print("ERROR: Requested checkpoint has no LoRA adapter weights")
            print(f"{'='*70}")
            print(f"Checkpoint: {args.checkpoint_dir}")
            print("Expected adapter_model.safetensors or adapter_model.bin.")
            print("Refusing to fall back to the base model under a checkpoint label.")
            print(f"{'='*70}\n")
            raise SystemExit(1)
        ...
        try:
            ...
            print(f"ERROR: Could not create LoRA request for {args.checkpoint_dir}: {exc}")
    ...
    if args.output_file is None:
        parts = ["eval_results", args.dataset]
        if dataset_file_sha256 is not None:
            parts.append(f"data{dataset_file_sha256[:12]}")
        parts.append(Path(args.base_model).name)
    ...
    print(f"Sampling seed: {args.seed if args.seed is not None else 'vLLM default'}")
